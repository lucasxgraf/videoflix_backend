...
class HlsSegmentTest(APITestCase):
    def setUp(self):
        ...
        self.url = reverse('hls-segment', kwargs={'movie_id': self.video.id, 'resolution': self.resolution, 'segment': self.segment})
    ...
    def test_hls_segment_path_traversal(self):
        url = reverse('hls-segment', kwargs={'movie_id': self.video.id, 'resolution': self.resolution, 'segment': '..'})
        response = self.client.get(url)
